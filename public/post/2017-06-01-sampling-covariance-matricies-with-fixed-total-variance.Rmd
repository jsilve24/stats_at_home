...
In retrospect I believe that an alternative method of generating uniform random point 
on the positive orthant of a hypersphere would be to use the built in `unit_vector`
data type in Stan as follows
...
```
Here we have taken advantage of the fact that Stan imposes a uniform prior 
for parameters with an unspecified prior. 
