...
\eta_i &\sim \text{Normal}(\beta_0 + \beta_\text{1}x_{\text{scratched}} + \beta_\text{2}x_{\text{year}} + 
\beta_\text{3}x_{\text{price}}, \sigma^2)
\end{align}$$

We will use a default uniform prior on $\beta_0$ and we will place informative priors on the other coefficients. For example, since I believe that having a scratch would likely decrease the probability that a phone sold I will use the following prior $\beta_1 \sim \text{Normal}(-1, 1)$. I also use the default flat LKJ prior for the variance term $\sigma^2$. 
